...
const CustomAdsTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white p-3 rounded-xl shadow-lg border" style={{ borderColor: '#d1d8e0' }}>
        <p className="text-sm font-medium mb-2 text-neo-text">{formattedDate}</p>
...
const CustomContribTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-white p-3 rounded-xl shadow-lg border" style={{ borderColor: '#d1d8e0' }}>
        <p className="text-sm font-medium mb-2 text-neo-text">{label}</p>
        {payload.map((entry: any, index: number) => (
          <p key={index} className="text-xs" style={{ color: entry.color }}>
...
try {
        const data = await axiosInstance.get('/api/v1/programs/').then(r => r.data);
        setRawPrograms(Array.isArray(data) ? data : []);
      } catch (err: any) {
        setProgramsError(err.response?.status === 403 ? "Accès aux annonces non autorisé" : "Erreur lors du chargement des annonces");
      }

      try {
        const data = await axiosInstance.get('/api/v1/events/').then(r => r.data);
        setRawEvents(Array.isArray(data) ? data : []);
      } catch (err: any) {
        setEventsError(err.response?.status === 403 ? "Accès aux événements non autorisé" : "Erreur lors du chargement des événements");
      }

      try {
        const data = await axiosInstance.get('/api/v1/contributions/').then(r => r.data);
        setRawContributions(Array.isArray(data) ? data : []);
      } catch (err: any) {
        setContributionsError(err.response?.status === 403 ? "Accès aux contributions non autorisé" : "Erreur lors du chargement des contributions");
      }
  ...
  if (loading) {
    return (
      <div className="p-4">
        <h3 className="text-2xl font-semibold text-neo-text mb-6">
          Tableau de bord
        </h3>
  ...
  return (
    <div className="p-4 space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h3 className="text-2xl font-semibold text-neo-text">
          Tableau de bord
        </h3>
        {/* Filtres de date */}
        <div className="flex items-center gap-4 neo-card-sm p-3">
          <div className="flex items-center gap-2">
            <label htmlFor="start-date" className="text-sm font-medium text-neo-text">
              Du
            </label>
            <input
              type="date"
          ...
          <div className="flex items-center gap-2">
            <label htmlFor="end-date" className="text-sm font-medium text-neo-text">
              Au
            </label>
            <input
              type="date"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="neo-card p-5">
          <h4 className="text-base font-bold text-neo-text mb-4">
      ...
      {!contributionsError && (
        <div className="neo-card p-5">
          <h4 className="text-base font-bold text-neo-text mb-4">Contributions</h4>
